...
def pre_process(images, down_factor):
    now_size = images.shape[2]
    aim_size = now_size / down_factor
    trans = transforms.Compose([transforms.RandomCrop(aim_size), transforms.Resize(now_size)])
...
def train_one_epoch(epoch, total_steps, dataloader, model,
                    ...
                    display_iter, log_file, show, warmup_step, warmup_method, down_factor):
    model.train()
    ...
    for inputs in dataloader:
        ...
        crop_imgs, crop_labels = pre_process_il(images, labels)
        # forward pass
        if down_factor >= 2:
            input_index = down_factor - 2
            pred = model(crop_imgs[input_index])
            loss = criterion(pred, crop_labels[input_index])
        elif down_factor == 1:
            pred = model(images)
...
def eval_one_epoch(epoch, threshold, dataloader, model, device, log_file, down_factor):
    with torch.no_grad():
        model.eval()
...
def train_eval_model(opts):
    ...
    shutil.copy("./train_unet_ablation.py", os.path.join(model_dir, "train_unet_ablation.py"))
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        epoch_acc, epoch_iou, epoch_f1 = eval_one_epoch(epoch, threshold, dataloaders_dict['eval'],
                                                        model, device, log_file, down_factor)
        ious.append(epoch_iou)

        if best_iou < epoch_iou and epoch >= 5:
            best_iou = epoch_iou
    ...
    plt.xlabel("epoch")
    plt.ylabel("iou")
    plt.title("Train, lr=" + str(lr) + '; warmup step=' + str(warmup_step) + '; warmup method=' + str(
        warmup_method))
    plt.legend()
...
if __name__ == '__main__':
    ...
    log_dir = "./train_log/" + str(opts["dataset_type"]) + "1_train_Unet_factor_" + str(
        opts["down_factor"]) + "_ablation_iou_" + str(
