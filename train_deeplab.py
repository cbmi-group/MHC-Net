...
def eval_one_epoch(epoch, dataloader, model, device, train_dir, log_file):
    with torch.no_grad():
        ...
        message = "total Threshold: {:.3f} =====> Evaluation IOU: {:.4f}; F1_score: {:.4f}; Haussdorf distance:{:.2f}".format(
...
def train_eval_model(opts):
    ...
    shutil.copy("./train_deeplab.py", os.path.join(model_dir, "train_deeplab.py"))
...
if __name__ == '__main__':
    ...
    log_dir = "./train_log/" + str(opts["dataset_type"]) + "_train_Deeplab_" + str(
        opts["train_batch_size"]) + '_' + str(
        opts["lr"]) + '_' + str(
        opts["num_epochs"]) + '_' + date + '_' + str(opts["weight_decay"]) + '_iouloss_warmup'
